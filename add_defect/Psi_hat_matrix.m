...
if omega==0 % size(Psi_hat_omega) = (floor(h/2)+1,k-1)
...
elseif omega>=1 && omega<=floor(N/(2*h)-1/2) % size(Psi_hat_omega) = (floor(h/2),1)
    ...
    Psi_hat_omega = Psi_hat(p + p_shift,1:k-1);
%     Psi_hat_omega = real(Psi_hat(p + p_shift,1:k-1)) + imag(Psi_hat(p + p_shift,1:k-1));
